...
// yaha error hai iski wajah se agla code bhi run nhi hora hai
...
const renderRecipe = data => {
    ...
    //     if (pair[0] === 'time' || 'ingr' || 'calories' || "diet" || "cuisine" || "meal" ||"health" || "dish") {
    ...
    const queryString = queries && queries.length > 0
    ? queries
        .map(pair => {
            if (pair[0] === 'time' || 'ingr' || 'calories' || "diet" || "cuisine" || "meal" || "health" || "dish") {
                pair[1] = pair[1].replace(/%20/g, '');
